...
class PackingService:
    ...
    # 修改 app/services/packing_service.py 中的 import_file_and_pack 方法
def import_file_and_pack(self, file_content: bytes, filename: str) -> OrderSummaryVo:
    if filename.endswith(('.xlsx', '.xls')):
        ...
        for enc in ['gbk', 'gb18030', 'utf-8-sig', 'utf-8']:
            try:
                ...
                continue
        if df is None:
            raise ValueError("CSV 文件编码无法识别，请使用 GBK 或 UTF-8 编码")
    else:
        raise ValueError("不支持的文件格式，仅支持 .xlsx, .xls, .csv")

    required_cols = {'房间', '成品长度', '成品宽度', '厚度', '板件名称', '板件条码'}
    if not required_cols.issubset(set(df.columns)):
        raise ValueError(f"文件缺少必要列，需包含: {required_cols}")

    boards: List[Board] = []
    for _, row in df.iterrows():
        thickness = int(row['厚度'])
        density = 12.0 if thickness == 9 else (15.0 if thickness == 12 else (28.0 if thickness == 22 else (30.0 if thickness == 25 else 23.0)))
        b = Board(
    ...
    def process_scan(self, dto: BoardScanDto) -> Tuple[bool, Optional[ScanGuideVo], str]:
        ...
        if not target_board:
            return False, None, f"条码 [{dto.barcode}] 不属于当前工单的任何包装！"
        ...
        if target_layer > 0:
            ...
            unscanned_lower = [b for b in lower_boards if not b["is_scanned"]]
            if unscanned_lower:
                vo = ScanGuideVo(
