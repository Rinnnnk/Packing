<template>
  <div class="h-screen w-screen flex flex-col bg-apple-bg text-apple-ink overflow-hidden font-sans select-none antialiased">
    <!-- 顶部导航 -->
    <HeaderBar
...
const currentPackage = computed<PackageVo | null>(() => {
  ...
  return orderSummary.value.packages.find(p => p.package_id === selectedPackageId.value) || null;
});
...
useBarcodeScanner(async (barcode: string) => {
  ...
  if (res.code === 200 && res.data) {
    ...
    const pkg = orderSummary.value.packages.find(p => p.package_id === res.data!.package_id);
    if (pkg) {
